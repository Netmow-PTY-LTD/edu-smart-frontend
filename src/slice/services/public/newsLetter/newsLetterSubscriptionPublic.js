import { serverInfo } from '@/utils/common/serverInfo';
import { createApi, fetchBaseQuery } from '@reduxjs/toolkit/query/react';
import Cookies from 'js-cookie';

export const newsLetterSubscriptionPublic = createApi({
  reducerPath: 'newsLetterSubscriptionPublic',
  baseQuery: fetchBaseQuery({
    baseUrl: `${serverInfo?.base_url_prod}` + '/api/v1/public',
    prepareHeaders: (headers) => {
      const token = Cookies.get('token');
      if (token) {
        headers.set('authorization', `Bearer ${token}`);
      }
      return headers;
    },
  }),
  endpoints: (builder) => ({
    sendSubscriptionEmail: builder.mutation({
      query: (body) => {
        return {
          url: `/newsletter/email/verify`,
          method: 'POST',
          body,
        };
      },
    }),
    subscribeNewsLetter: builder.mutation({
      query: (body) => {
        return {
          url: `/newsletter/subscribe`,
          method: 'POST',
          body,
        };
      },
    }),
    unSubscribeNewsLetter: builder.mutation({
      query: (body) => {
        return {
          url: `/newsletter/unsubscribe`,
          method: 'POST',
          body,
        };
      },
    }),
  }),
});

export const {
  useSendSubscriptionEmailMutation,
  useSubscribeNewsLetterMutation,
  useUnSubscribeNewsLetterMutation,
} = newsLetterSubscriptionPublic;
