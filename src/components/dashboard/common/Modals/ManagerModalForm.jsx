...
import { emptyManager } from '@/slices/dashboard/adminDashboard/reducer';
...
const ManagerModalForm = ({
  ...
  const {
    ...
    error: managerError,
  } = useSelector((state) => state.AdminDashboard.addManager);

  useEffect(() => {
    dispatch(emptyManager());
  }, [dispatch]);

  useEffect(() => {
    if (managerData?.message && managerError === null) {
  ...
  const handleManagerSubmit = (e) => {
    ...
    Object.entries(managerAllData).forEach(([key, value]) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      dispatch(addManager(treamedData));
      if (managerData?.message && managerError === null) {
        toast.success(managerData?.message);
