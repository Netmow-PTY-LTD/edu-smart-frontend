...
import { allShopCategory } from '@/slices/dashboard/ecommerce/Actions/shopActions';
...
const EditProductFormModal = ({
      ...
      if (res.error) {
        ...
        if (allProductCategory) {
          const categoryData =
            ...
            allProductCategory.map((category) => ({
              label: category?.name,
              value: category?._id,
            }));
  ...
  }, [allProductCategory.length > 0, dispatch]);
  ...
  const handleAddField = (e, name) => {
    ...
    if (name === 'colors') {
      ...
      setFormData({
        ...formData,
        colors: allColorsObject,
      });
    }
  };

  const handleRemoveField = (id, name) => {
    if (id !== undefined && name === 'colors' && formData.colors.length > 1) {
      const updatedDates = formData.colors.filter((d) => d.id !== id);
  ...
  const handleChooseColors = (value, id) => {
    const updatedColors = formData.colors.map((col) =>
      col.id === id ? { ...col, color: value } : col
    );
            ...
            <Row>
              ...
              <Col lg={12}>
                <div className="mb-3">
                  ...
                  />
                  {errors?.image && (
                    <div className="text-danger fs-3 mt-2">{errors?.image}</div>
                  )}
              ...
              <Col lg={4}>
                <Row>
                  {formData?.colors?.length > 0 &&
                    formData.colors.map((cols, i) => (
                      <React.Fragment key={cols.id}>
                        <Col lg={6}>
                          <div className="mb-3">
                            <Label
                              htmlFor={`colorsInput${i}`}
                            ...
                            <input
                              ...
                              value={cols.color}
                              onChange={(e) =>
                                handleChooseColors(e.target.value, cols.id)
                              }
                            />
                          </div>
                        </Col>
                        ...
                        <Col lg={3}>
                          <div className="d-flex flex-column text-nowrap fs-4">
                            <p>
                              {cols.color &&
                              /^#([0-9A-F]{3}){1,2}$/i.test(cols.color)
                                ? ColorNamer(cols.color).ntc[0].name
                                : 'Name'}
                            </p>
                            <div
                              className="rounded-circle"
                              style={{
                                backgroundColor: cols.color || '',
                                width: '45px',
                                height: '45px',
