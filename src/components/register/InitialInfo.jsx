...
import { emptySendOtp } from '@/slices/dashboard/adminDashboard/reducer';
...
const InitialInfo = ({
  ...
  const { data, isLoading, error } = useSelector(
    (state) => state.AdminDashboard.sendOtp
  );
  ...
  const handleStepsAndData = async (e) => {
    ...
    if (isValid) {
      ...
      try {
        await dispatch(sendOtp(otpData)).then((res) => {
          dispatch(emptySendOtp());
        });
      } catch (error) {
  ...
  const googleRegistrationHandler = (e) => {
    ...
    if (subdomain && subdomain !== 'localhost') {
      if (userRole) {
        router.push(
          `${process.env.NEXT_PUBLIC_BASE_URL_PROD}/public/api/v1/auth/with-google?usertype=${userRole}&subdomain=${subdomain}`
        );
      } else {
        setUserRoleError('Please Select An User.');
        setTimeout(() => {
      ...
      router.push(
        `${process.env.NEXT_PUBLIC_BASE_URL_PROD}/public/api/v1/auth/with-google?usertype=admin&subdomain=none`
      );
    }
  };
  ...
  const linkedinRegistrationHandler = (e) => {
    ...
    if (subdomain && subdomain !== 'localhost') {
      if (userRole) {
        router.push(
          `${process.env.NEXT_PUBLIC_BASE_URL_PROD}/public/api/v1/auth/with-linkedin?usertype=${userRole}&subdomain=${subdomain}`
        );
      } else {
        setUserRoleError('Please Select An User.');
        setTimeout(() => {
      ...
      router.push(
        `${process.env.NEXT_PUBLIC_BASE_URL_PROD}/public/api/v1/auth/with-linkedin?usertype=admin&subdomain=none`
      );
    }
  };
  ...
  return (
    <>
      ...
      <GoogleOAuthProvider clientId="614492324409-f8l9j8dnspdgegrpl1up06ms6raefc21.apps.googleusercontent.com">
        <div className="p-4">
          <div className="">
            <h5 className="fs-1 fw-semibold">
              {!subdomain
                ? 'Register Your Sports Team, Organization, or Club'
                : 'Register Account'}
            </h5>
            <p className="text-muted fs-3">
              {!subdomain ? 'Get your Free Team SquadDeck account' : ''}
            </p>
          </div>
              ...
              {subdomain === 'localhost' || !subdomain ? (
                <div className="mb-3 form-check">
                  <input
                    ...
                    required
                  />
                  <label className="form-check-label fs-3" htmlFor="termsCheck">
                    I agree to the{' '}
              ...
              <div className="mt-3">
                {isLoading ? (
                  ...
                  <button
                    ...
                    className={`button btn text-light w-100 fs-2 py-3`}
                  >
                    Register
                  </button>
                )}
                ...
                {error && (
                  <div className="text-danger text-center fs-2 mt-4">
                    {error}
                  </div>
                )}
              </div>
