...
export default function UniversitySliderList({ university_id }) {
  ...
  const validationSchema = Yup.object({
    ...
    button_2_link: Yup.string()
      ...
      .test('fileType', 'Unsupported file format', (value) => {
        ...
        return value
          ? ['image/jpeg', 'image/jpeg','image/png', 'image/gif'].includes(value.type)
          : true;
      })
  ...
  const handleEditButtonClick = (id) => {
    const slider = getSingleUniversityData?.data?.sliders?.find(
  ...
  const handleUpdateslider = async (values, { setSubmitting }) => {
    ...
    const updatesliderData = { data:{id: sliderId, ...values} , university_id: university_id };
    
    try {
    //   const result = await universityslider(updatesliderData).unwrap();
    //   if (result) {
    //     toast.success(result?.message);
  ...
  const slidersHeaders = [
    ...
    { title: 'Slider Picture', key: 'image' },
