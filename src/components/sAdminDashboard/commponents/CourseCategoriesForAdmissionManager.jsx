...
} from '@/slice/services/admission manager/courseCategoriesServiceForAdmissionManager';
...
const CourseCategoriesForAdmissionManager = ({
  university_id,
  allDepartmentData,
}) => {
  ...
  const [addCourseCategoryForAdmissionManager] =
    useAddCourseCategoryForAdmissionManagerMutation();

  const {
    data: getAllCourseCategoriesForAdmissionManagerData,
    error: getAllCourseCategoriesForAdmissionManagerError,
    isLoading: getAllCourseCategoriesForAdmissionManagerIsLoading,
    refetch: getAllCourseCategoriesForAdmissionManagerRefetch,
  ...
  const [updateCourseCategoryForAdmissionManager] =
    useUpdateCourseCategoryForAdmissionManagerMutation();

  const [
    deleteCourseCategoryForAdmissionManager,
    {
      data: deleteCourseCategoryForAdmissionManagerData,
      error: deleteCourseCategoryForAdmissionManagerError,
      isLoading: deleteCourseCategoryForAdmissionManagerIsLoading,
    },
  ] = useDeleteCourseCategoryForAdmissionManagerMutation();
  ...
  useEffect(() => {
    if (
      ...
      const getSingleCategoryData =
        getAllCourseCategoriesForAdmissionManagerData?.data?.length > 0
          ? getAllCourseCategoriesForAdmissionManagerData?.data.find(
  ...
  }, [getAllCourseCategoriesForAdmissionManagerData?.data, categoryIdForEdit]);
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      const result = await addCourseCategoryForAdmissionManager({
        ...values,
        university_id: university_id,
      }).unwrap();
      if (result) {
        toast.success(result?.message);
        getAllCourseCategoriesForAdmissionManagerRefetch();
  ...
  const handleUpdateCategory = async (values, { setSubmitting }) => {
    ...
    try {
      const result =
        await updateCourseCategoryForAdmissionManager(finalData).unwrap();
      if (result) {
        toast.success(result?.message);
        getAllCourseCategoriesForAdmissionManagerRefetch();
  ...
  const handleDeleteCategory = async (id) => {
    try {
      const result = await deleteCourseCategoryForAdmissionManager({
        university_id: university_id,
        category_id: id,
      }).unwrap();
      if (result) {
  ...
  const isfilteredData =
    getAllCourseCategoriesForAdmissionManagerData?.data?.length > 0 &&
    getAllCourseCategoriesForAdmissionManagerData?.data.filter((item) =>
  ...
  return (
    <>
      <div className="container-fluid">
        <div className="h-100">
          <ToastContainer />
          {getAllCourseCategoriesForAdmissionManagerIsLoading ? (
            <LoaderSpiner />
          ) : (
            <Card>
...
export default CourseCategoriesForAdmissionManager;
