...
const AddCategoryService = async (data) => {
  ...
  const res = await axios.post(
    process.env.NEXT_PUBLIC_BASE_URL_PROD + '/api/v1/news/categories',
    data,
    {
      headers: {
        authorization: token,
...
const AddNewsService = async (data) => {
  const token = localStorage.getItem('token');
  const res = await axios.post(
    process.env.NEXT_PUBLIC_BASE_URL_PROD + '/api/v1/news',
    data,
    {
      headers: {
        authorization: token,
...
const getSingleNews = async (id) => {
  const token = localStorage.getItem('token');
  const url =
    process.env.NEXT_PUBLIC_BASE_URL_PROD + `/api/v1/news/${id}`;
...
const updateNewsService = async (data) => {
  const token = localStorage.getItem('token');
  const url =
    process.env.NEXT_PUBLIC_BASE_URL_PROD + `/api/v1/news/${data.id}`;
